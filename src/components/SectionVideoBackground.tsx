...
const SectionVideoBackground = ({
  ...
  const shouldRenderVideo = shouldEnableVideo && !hasVideoError && !!activeTheme.src;
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        if (bestRatio > 0) {
          setActiveSection((prev) => (prev === bestId ? prev : (bestId as VideoSection)));
        }
      },
      {
        root: null,
